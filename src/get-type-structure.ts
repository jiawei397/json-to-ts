...
function Hash(content: string): string {
  return sha1(content, "utf8", "hex").toString();
}
...
function getMergedUnion(
  ...
  const primitiveTypes = typesOfArray.filter((id) =>
    !findTypeById(id, types) || !findTypeById(id, types).isUnion
...
function getInnerArrayType(
  ...
  const arrayTypesDescriptions = typesOfArray.map((id) =>
    findTypeById(id, types)
  ).filter((_) => !!_);

  const allArrayType =
    arrayTypesDescriptions.filter((typeDesc) =>
      getTypeDescriptionGroup(typeDesc) === TypeGroup.Array
    ).length ===
      typesOfArray.length;
  ...
  const allObjectTypeWithNull =
    arrayTypesDescriptions.filter((typeDesc) =>
            getTypeDescriptionGroup(typeDesc) === TypeGroup.Object
          ).length + 1 ===
      typesOfArray.length && containsNull;
  ...
  if (allArrayTypeWithNull) {
    return getMergedUnion([
      getMergedArrays(arrayTypesDescriptions, types),
      "null",
    ], types);
  }
  ...
  if (allObjectTypeWithNull) {
    return getMergedUnion([
      getMergedObjects(arrayTypesDescriptions, types),
      "null",
    ], types);
  }
...
export function getTypeStructure(
  ...
  switch (getTypeGroup(targetObj)) {
    case TypeGroup.Array:
      const typesOfArray = (<any[]> targetObj).map((_) =>
        getTypeStructure(_, types).rootTypeId
      ).filter(onlyUnique);
...
function getAllUsedTypeIds({ rootTypeId, types }: TypeStructure): string[] {
  ...
  const subTypes = (typeDesc: TypeDescription): string[] => {
